...
class realb{
    ...
    async getImages(tag, page_amount, image_amount, filter){
        ...
        for(let i = 0; i < page_amount; i++){
            const page = await this.getData(`${this.baseURL}/index.php?page=dapi&s=post&q=index&pid=${i}&limit=100&json=1&tags=${tag}`)
            if(page.length == 0) break;
            page.forEach(img => {
                imgurl_list.push(`${this.baseURL}/images/${img.directory}/${img.image}`)
            })
        }
